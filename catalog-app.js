(() => {
  ...
  function updateCatalogRoute(historyMode = "replace") {
    ...
    if (!window.KITRADE_PREVIEW_MODE && window.location.protocol !== "file:" && historyMode !== "none") {
  ...
  function updateRequestSubmitState() {
    ...
    const hasRequestContent = state.selected.length > 0 || Boolean(requestMissingPart?.value.trim());
    requestSubmit.setAttribute("data-ready", String(hasRequestContent));
  }

  function showToast(message) {
    toast.textContent = message;
    toast.setAttribute("data-cart-success", String(message === "Запчасть добавлена в корзину"));
    clearTimeout(toastTimer);
    clearTimeout(toastRevealTimer);
    ...
    const waitForLanding = () => {
      if (document.body.classList.contains("catalog-cart-flight-active")) {
        toastRevealTimer = setTimeout(waitForLanding, 30);
      } else reveal();
    };
    toastRevealTimer = setTimeout(waitForLanding, 0);
  }
  ...
  function clearCatalogQuery() {
    ...
    state.query = "";
    document.querySelector("#catalogSearch > button")?.setAttribute("data-ready", "false");
  }
  ...
  const searchSubmit = document.querySelector("#catalogSearch > button");
  let searchTimer;
  const updateSearchSubmitState = () => {
    searchSubmit?.setAttribute?.("data-ready", String(Boolean(searchInput.value.trim())));
  };
  const applySearch = () => {
  ...
  document.addEventListener("kitrade:add-product", (event) => {
    ...
    renderRequest();
    render();
    showToast("Запчасть добавлена в корзину");
  });
  try {
  ...
  try {
    ...
    const returning = sessionStorage.getItem('kitradeRestoreCatalog') === location.pathname;
    sessionStorage.removeItem('kitradeRestoreCatalog');
    if ((!hasExplicitInitialFilters || returning) && saved?.path === location.pathname) {
      clearFilterControls();
