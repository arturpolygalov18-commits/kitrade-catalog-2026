...
for (const product of registry.entities.products) {
  ...
  assert.ok(html.includes("data-product-add") && html.includes("basket-checkout.js"), `Missing request control for ${product.canonical_path}`);
...
for (const [relativePath, expectedBodyAttributes] of catalogRouteCases) {
  ...
  assert.ok(html.includes('catalog-app.js?v=45'), `Catalog route ${relativePath} does not use the cache-busted filter script`);
}
...
for (const value of ['"@type":"AutoPartsStore"', '"name":"Китрейд"', '"legalName":"ИП Заварзин Дмитрий Александрович"', '"email":"kitrade@bk.ru"', '"telephone":"+7-995-245-30-00"', '"streetAddress":"пр-т Ленина, д. 3"']) {
