...
const commercialRows = seoMap.map((row) => {
  ...
  const hasRequest = /(?:href=["'][^"']*#request|data-product-request|data-product-add|data-request-form|id=["']request["'])/i.test(html);
  const hasPhone = /href=["']tel:\+?\d+/i.test(html);
  const hasMinOrder = /50\s*000\s*₽/u.test(html);
  const explainsDelivery = /отдельно\s+рассчитаем\s+доставку|доставк[а-яё]*\s+(?:рассчитывается|рассчитает|отдельно)|цена\s*[—–-]\s*за деталь/iu.test(html);
