...
test("generated current-catalog YML is well formed, unique, absolute and fully accounted for", () => {
  ...
  assert.equal(report.counts.advertising_filter_preview, 493);
  assert.equal(report.counts.advertising_filter_with_article, 453);
  assert.equal(report.counts.advertising_filter_without_article, 40);
  assert.equal(report.counts.advertising_filter_name_contains_article, 453);
  assert.equal(report.counts.advertising_filter_priority_prefix, 493);
  assert.equal(report.counts.descriptions_with_oem_prefix, 0);
  assert.ok(offerIds.slice(0, report.counts.advertising_filter_priority_prefix).every((id) => {
  ...
  assert.ok(fs.existsSync(path.join(projectDir, "assets", "catalog-products", "1184-p01-c3f1ed78b3fb.webp")));
  assert.match(xml, /<offer id="1184" available="false">[\s\S]*?<picture>https:\/\/xn--d1abifc1bn\.xn--p1ai\/assets\/catalog-products\/1184-p01-c3f1ed78b3fb\.webp<\/picture>[\s\S]*?<name>Передняя фара правая Geely Galaxy L7 6608086101<\/name>[\s\S]*?<vendorCode>6608086101<\/vendorCode>[\s\S]*?<custom_label_0>Новое<\/custom_label_0>[\s\S]*?<\/offer>/);
