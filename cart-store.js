  ...
  window.KITRADE_CART = {
    ...
    add(product) {
      ...
      put(id, { id, active: true, quantity: current?.quantity || 1, title: product.title, article: product.article || '',
        canonicalPath: product.canonicalPath || product.canonical_path || '/catalog/',
        price: product.priceNumber ?? product.price ?? current?.price ?? 0,
        photos: product.photos || (product.image ? [product.image] : current?.photos || []), addedAt: Date.now() });
      notify();
    },
